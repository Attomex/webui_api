...
const Search = () => {
    ...
    useEffect(() => {
      ...
      const computerName = searchParams.get("computer_name");
  
      // Формируем поисковый запрос из параметров
      let queryParts = [];
      if (reportNumber) queryParts.push(`report_number: ${reportNumber}`);
      if (reportDate) queryParts.push(`report_date: ${reportDate}`);
      if (computerName) queryParts.push(`computer_name: ${computerName}`);
    ...
    const filtersConfig = [
        {
            ...
            label: "Название ПК",
            maxCount: 1,
            hint: "Добавить фильтр по названию ПК",
        },
        {
            key: "report_date",
            label: "Дата отчёта",
            maxCount: 1,
            hint: "Добавить фильтр по дате отчёта",
        },
        {
            key: "vulnerability_id",
            label: "Идентификатор уязвимости",
            maxCount: 2,
            hint: "Добавить фильтр по идентификатору уязвимости",
        },
        {
            key: "filename",
            label: "Название файла",
            maxCount: 1,
            hint: "Добавить фильтр по названию файла",
        },
        {
            key: "report_number",
            label: "Номер отчёта",
            maxCount: 1,
            hint: "Добавить фильтр по номеру отчёта",
        },
    ];
    ...
    const handleAddFilter = (filterKey) => {
        ...
        const currentCount = (
            searchQuery.match(new RegExp(`${filterKey}:`, "g")) || []
        ).length;

        if (currentCount >= filter.maxCount) {
            alert(
                `Максимальное количество добавлений для "${filter.label}" — ${filter.maxCount}`
            );
            return;
        }
    ...
    const handleSearchSubmit = async (e) => {
        ...
        try {
            ...
            for (const filter of filtersConfig) {
                const currentCount = (
                    searchQuery.match(new RegExp(`${filter.key}:`, "g")) || []
                ).length;
                if (currentCount > filter.maxCount) {
                    isValid = false;
                    alert(
                        `Превышено максимальное количество добавлений для "${filter.label}" — ${filter.maxCount}`
                    );
                    break;
                }
            }
    ...
    return (
        <div className={c.container}>
            ...
            <div className={c.hint}>
                ...
                <code>computer_name: pc_34dddw, report_date: 28-12-2024</code>
            </div>
            {/* Форма поиска */}
